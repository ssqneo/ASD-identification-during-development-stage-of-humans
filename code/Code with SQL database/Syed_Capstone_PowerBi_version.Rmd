...
# The following chunk saves all in the same folder as this script which could be later imported in to PowerBi to create interactive visualization dashboards.

```{r}
install.packages("xlsx")
require(xlsx)

# Use the getwd command to get the working directory, that is where all the files will be saved.
getwd()

# You can change the working directory by specifing a path in quotes and forwardslash. See line 1263 for an example.
# setwd("C:/Users/STU/Documents")

write.xlsx(Genes_merged, "Genes_merged.xlsx", col.names = T, row.names = T, append = F)

write.xlsx(delta_top_20, "Delta_top_20.xlsx", col.names = T, row.names = T, append = F)

write.xlsx(Diff_AVG_ASD, "Diff_AVG_ASD.xlsx", col.names = T, row.names = T, append = F)

write.xlsx(Diff_AVG_WASD, "Diff_AVG_WASD.xlsx", col.names = T, row.names = T, append = F)

# The datasets saved above can be imported into powerbi by going to PowerBi -> Home -> Get data -> Excel 
